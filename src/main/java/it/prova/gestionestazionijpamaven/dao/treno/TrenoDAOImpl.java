...
public class TrenoDAOImpl implements TrenoDAO {
	...
	@Override
	public Treno findByDenominazione(String denominazioneInput) throws Exception {
		TypedQuery<Treno> query = entityManager
				.createQuery("select g from Treno g where g.denominazione=?1", Treno.class)
...
//	######################################################################################################
	...
	@Override
	public List<Integer> loadListaNumeroAbitantiAssociatiAdUnaStazione(Long idStazioneInput) throws Exception {
		TypedQuery<Integer> query = entityManager.createQuery(
				"select c.numeroAbitanti FROM Citta c left join c.stazioni s left join s.treni t where t.id = :idTreno",
				Integer.class);
		query.setParameter("idTreno", idStazioneInput);
